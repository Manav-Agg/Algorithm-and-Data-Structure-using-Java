...
public class SplayTree<T extends Comparable<T>> implements Tree<T>{
    ...
    private void rotateLeft(Node<T> node){
        ...
        if(node.getParentNode() == null){
            ...
            node.getParentNode().setParentNode(tempNode);
        }
        else{
...
//                    so the node is a left child + grandparent is null
                    rotateRight(node.getParentNode());
                }
                else{
//                    the node is a right child + grandparent is null
...
//                so left child and parent is a left child too
...
//                so right child and parent is a right child too
                rotateLeft(node.getParentNode().getParentNode());
                rotateLeft(node.getParentNode());
            }

//            ZIG-ZAG Situation
            else if(node.getParentNode().getLeftNode() == node && node.getParentNode().getParentNode().getParentNode() == node.getParentNode()){
//                zigzag: so left child and parent is a right child
                rotateRight(node.getParentNode());
                rotateLeft(node.getParentNode());
            }
            else{
//                this is the zigzag situation: so right child and parent is a left child
                rotateLeft(node.getParentNode());
                rotateRight(node.getParentNode());
            }
        }
